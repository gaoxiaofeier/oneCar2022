<template>
    <el-main>
                ...
                <el-table :data="tableData" style="width: 100%" border>
                    ...
                    <el-table-column label="操作" width="120" fixed='right'>
                        <template slot-scope="scope">
                            ...
                            <el-button type="text" size="small" v-else @click="publish(scope.row.actId, scope.row.status)" style="margin-left:0px;color:#E6A23C">
                                上架
                            </el-button>
                ...
                <ul>
                    ...
                    <div style="margin-bottom:20px;">
                        ...
                        <el-form-item label="条款链接文件" style="margin-top:20px;">
                            <el-upload :action="this.action" :http-request="Upload"  :before-upload="beforeClauseUpload" :on-success="successAdd" :show-file-list="false">
                                <el-button size="small" type="primary">点击上传</el-button>
                            </el-upload>

                            <div v-if="this.ruleForm.termsUrl" class="clause">
                                <span><i class="el-icon-document"></i>{{this.fileName||this.ruleForm.termsUrl || '条款名称.html'}}</span><i class="el-icon-delete" @click="deleteClause" style="margin-left:6px"></i>
                            </div>
        ...
        <!-- 确认删除弹框 -->
        <el-dialog :visible.sync="conformDeleteVisible" width="300px"  title="提示">
            <p style="text-align:center">确认删除？</p>
            <div slot="footer" class="dialog-footer" style="vertical-align: middle;text-align:center;">
...
export default {
    ...
    data() {
        return {
            ...
            fileAction:'https://app.onecar.fun/api/v2.0/common/uploadToServer',    
            search: {
                actName: '',
    ...
    methods: {
        ...
        async Upload(file) {//上传文件
            ...
            let param = new FormData()  // 创建form对象
              param.append('file', file.file, file.name)  // file对象是 beforeUpload参数
              let config = {
                headers: {'Content-Type': 'multipart/form-data'}
              }
            util2.ajax
                .post('common/uploadToServer', param,config)
                .then((res) => {
                    console.log(res.data.code)
                    if(res.data.code==200){
                         this.fileName = ''
                    this.ruleForm.termsUrl = res.data.data
                    this.$forceUpdate()
                    }else{
                ...
                if (valid) {
                    ...
                    template.collectItems.forEach((item, index) => {
                        if(item.type!=""){
                            ...
                            if(item.actId!=''){
                                template.delActCollectItemIds.push(item.actId)
                            }
                             template.collectItems.splice(index,1)
                        }
                    })
                    ...
                    util2.ajax
                        ...
                        .then((res) => {
                            ...
                            this.ruleForm.delActCollectItemIds=[]
                            //文件上传，文件路径变量清空
                            this.ruleForm.termsUrl=''
                            this.fileName=''
                        })
